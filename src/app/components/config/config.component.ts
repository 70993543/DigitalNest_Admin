...
export class ConfigComponent implements OnInit {
  ...
  constructor(private _adminService: AdminService) {
    ...
    this._adminService.obtener_config_admin(this.token).subscribe({
      next: (response) => {
        ...
        this.url = GLOBAL.url + 'obtener_logo/' + this.config.logo; // Corregir la URL

        // También podrías establecer la imagen seleccionada aquí si es necesario
        this.imgSelect = this.url;
      },
      error: (error) => {
        console.log(error);
      },
  ...
  actualizar(confForm: NgForm) {
    if (confForm.valid) {
      ...
      console.log(data);
      this._adminService
        .actualiza_config_admin('657782fedd5f2bebac36c858', data, this.token)
        .subscribe({
          next: (response) => {
            console.log(response);
            iziToast.show({
